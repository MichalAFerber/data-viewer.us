(function(){
  ...
  var ACCEPT_EXT = {
    json:1, jsonc:1, json5:1, jsonld:1, ndjson:1, yaml:1, yml:1, toml:1,
    csv:1, tsv:1, xml:1, rss:1, atom:1, graphql:1, gql:1
  };
  ...
  function buildRendered(){
    ...
    try {
      if (renderKind === "table"){
        ...
        renderTable(parseCsv(rawText, ext === "tsv" ? "\t" : ","));
      } else if (renderKind === "xml"){
        renderXml(rawText);
      } else { // tree
        parsed = parseFor(extOf(currentName), rawText);
        renderTree(parsed);
      }
    } catch (e){
      parseErr = String(e && e.message || e);
      dataView.innerHTML = "";
      var p = el("div","parse-error"); p.textContent = "Couldn’t parse this file: " + parseErr + "  — showing source instead.";
      dataView.appendChild(p);
    }
    dataView.className = "dataview" + (renderKind === "table" ? "" : "");
    dataBuilt = true;
  }
  ...
  function show(text, name){
    rawText = text; currentName = name || "";
    syncQueryName(currentName);
    var ext = extOf(name);
    renderKind = RENDER_KIND[ext] || "";
    hasRendered = !!renderKind || (!ext && looksStructured(text));
    if (hasRendered && !renderKind) renderKind = "tree";
    parsed = null; parseErr = ""; dataBuilt = false; codeBuiltFor = null;
  ...
  doc.addEventListener("keydown", function(e){
    if (!id("routeCard").hidden){              // §6.10 offer card is modal: Escape closes, Tab cycles its two buttons
  ...
  // ---------- Family router (§6.10): wrong-viewer offer card + in-browser hand-off ----------
  ...
  // Receiver — a sibling family tab (or the hub) hands the File across via postMessage.
  ...
  if (fvh){
    ...
    if (window.opener){
      ...
      if (emptySub){
        var emptySubIdle = emptySub.textContent;
        emptySub.textContent = "Receiving \u201C\u2068" + fvhName + "\u2069\u201D\u2026";
        setTimeout(function(){ emptySub.textContent = emptySubIdle; }, 10000);
      }
    }
  }
