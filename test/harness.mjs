...
// regardless of whether the content parses, so this is enough to exercise
// the ?name= write path without a real structured-data payload.
const FIX = mkdtempSync(join(tmpdir(), "data-harness-"));
const FIXTURE_JSON = join(FIX, "sample.json");
writeFileSync(FIXTURE_JSON, "{}");
...
await page.setInputFiles("#fileInput", FIXTURE_JSON);
await page.waitForFunction(() => document.body.classList.contains("viewing"), null, { timeout: 10000 });
check("load: URL reflects ?name=sample.json", await page.evaluate(() =>
  new URLSearchParams(location.search).get("name")) === "sample.json");
...
hook(p3);
await p3.goto(`http://localhost:${PORT}/?name=${encodeURIComponent("sample.json")}`, { waitUntil: "load", timeout: 30000 });
